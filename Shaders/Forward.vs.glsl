...
out vec3 v_positionInWorldSpace;
out vec4 v_positionInLightSpace;
out vec3 v_normal;
out vec2 v_texcoord;
...
void main()
{
    ...
    v_positionInWorldSpace = worldPosition.xyz;
	v_texcoord = in_texcoord.xy;
	v_normal = (u_model * vec4(in_normal, 0.0)).xyz;
    v_positionInLightSpace = u_light.transform * vec4(fragPosition, 1.0);
