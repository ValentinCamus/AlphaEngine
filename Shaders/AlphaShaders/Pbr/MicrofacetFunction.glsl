#ifndef MICROFACETFUNCTION_GLSL
#define MICROFACETFUNCTION_GLSL
/*
...
float CosDPhi(vec3 wa, vec3 wb)
{
    return clamp((wa.x * wb.x + wa.y * wb.y) /
                 sqrt((wa.x * wa.x + wa.y * wa.y) * (wb.x * wb.x + wb.y * wb.y)), -1, 1);
}
...
float TRD_Lambda(vec3 w, float alphax, float alphay)
{
    ...
    float alpha = sqrt(Cos2Phi(w) * Pow2(alphax) + Sin2Phi(w) * Pow2(alphay));
...
float TRD_D(vec3 wh, float alphax, float alphay)
{
    ...
    float cos4Theta = Pow2(Cos2Theta(wh));

    float e =
        ( Cos2Phi(wh) / Pow2(alphax) +
          Sin2Phi(wh) / Pow2(alphay) ) *
        tan2Theta;

    return (1.0f / (Pi * alphax * alphay * cos4Theta * Pow2(1 + e)));
}
