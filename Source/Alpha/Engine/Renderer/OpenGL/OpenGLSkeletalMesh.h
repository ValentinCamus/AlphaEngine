...
namespace Alpha
{
    class OpenGLSkeletalMesh : public SkeletalMesh
    {
    public:
        /// Load the mesh from data.
        explicit OpenGLSkeletalMesh(const std::vector<Vertex>& vertices, const std::vector<uint32>& indices);

        ~OpenGLSkeletalMesh() override { Destroy(); }
