...
namespace Alpha
{
    ...
    void OpenGLStaticMesh::SetupMesh()
    {
        ...
        // Vertex bone ids
        offsetOf = (void*) offsetof(Vertex, boneIds);
        GL_CHECK(glVertexAttribPointer(3, 3, GL_INT, GL_FALSE, sizeof(Vertex), offsetOf));

        // Vertex bone weights
        offsetOf = (void*) offsetof(Vertex, weights);
        GL_CHECK(glVertexAttribPointer(4, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), offsetOf));
